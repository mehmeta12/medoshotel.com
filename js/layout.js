  ...
  <nav class="nav-container" aria-label="Ana Navigasyon">
    ...
    <div class="nav-right">
      ...
      <div class="lang-toggle" role="group" aria-label="Dil / Language">
        <button class="lang-toggle-btn" data-set-lang="tr" aria-current="${(window.MEDOS && window.MEDOS.lang) === 'en' ? 'false' : 'true'}">TR</button>
        <button class="lang-toggle-btn" data-set-lang="en" aria-current="${(window.MEDOS && window.MEDOS.lang) === 'en' ? 'true' : 'false'}">EN</button>
      </div>
...
<footer class="site-footer">
  ...
  <div class="footer-bottom">
    <div class="footer-bottom-inner">
      ...
      <p class="footer-dev" data-i18n-html="footer.developer">${t('footer.developer')}</p>
    </div>
  </div>
...
<div class="cookie-banner" id="cookieBanner" role="dialog" aria-live="polite" aria-label="Çerez bildirimi" hidden>
  ...
  function buildCTA() {
    var bookLabel = t('btn.book') || 'Rezervasyon Yap';
    var chatLabel = t('cta.chat') || 'Asistanınız';
    return `
<style>
@media(max-width:600px){
  #cta-fixed{flex-direction:column!important;bottom:.75rem!important;right:.75rem!important;gap:6px!important;width:172px}
  #cta-fixed a,#cta-fixed button{height:44px!important;padding:0 12px!important;font-size:13px!important;width:100%;justify-content:center;box-sizing:border-box}
  .cookie-banner{bottom:168px!important;left:.75rem!important;right:.75rem!important;width:auto!important;max-width:calc(100% - 1.5rem)!important;border-radius:12px!important}
  #medos-chatbot-root{bottom:calc(.75rem + env(safe-area-inset-bottom,0px) + 165px)!important;right:.75rem!important}
  #medos-chatbot-root .mc-box{max-height:calc(100vh - 190px)!important;width:calc(100vw - 1.5rem)!important}
}
</style>
<div id="cta-fixed" style="position:fixed;bottom:1.5rem;right:1.5rem;z-index:9999;display:flex;flex-direction:row;gap:8px;align-items:center;">
  <a href="booking.html" style="display:inline-flex;align-items:center;gap:8px;background:#13508b;color:#fff;padding:0 20px;height:48px;border-radius:9999px;font-family:Kanit,sans-serif;font-size:14px;font-weight:600;text-decoration:none;white-space:nowrap;box-shadow:0 4px 16px rgba(0,0,0,.25);">
    ...
    <span data-i18n="btn.book">${bookLabel}</span>
  </a>
  <a href="tel:+902566450740" style="display:inline-flex;align-items:center;gap:8px;background:#00b1c9;color:#fff;padding:0 20px;height:48px;border-radius:9999px;font-family:Kanit,sans-serif;font-size:14px;font-weight:600;text-decoration:none;white-space:nowrap;box-shadow:0 4px 16px rgba(0,0,0,.25);">
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 1.18h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 8.91a16 16 0 0 0 7 7l.78-.78a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 17.5z"/></svg>
    0256 645 07 40
  </a>
  <button type="button" onclick="if(window.MedosChat)window.MedosChat.toggle()" style="display:inline-flex;align-items:center;gap:8px;background:#13508b;color:#fff;padding:0 20px;height:48px;border-radius:9999px;font-family:Kanit,sans-serif;font-size:14px;font-weight:600;border:none;cursor:pointer;white-space:nowrap;box-shadow:0 4px 16px rgba(0,0,0,.25);">
    ...
    <span data-i18n="cta.chat">${chatLabel}</span>
  </button>
</div>`;
  }
  ...
  function inject() {
    /* Google Fonts — Kanit (skip if already in <head>) */
    if (!document.querySelector('link[href*="Kanit"]')) {
      ...
      gf.rel = 'stylesheet';
      gf.href = 'https://fonts.googleapis.com/css2?family=Kanit:wght@300;400;500;600;700&display=swap';
    ...
    /* Load chatbot only on first user interaction (reduces unused JS on initial load) */
    var chatbotLoaded = false;
    function loadChatbot() {
      if (chatbotLoaded) return;
      chatbotLoaded = true;
      var s = document.createElement('script');
      s.src = 'js/chatbot.min.js';
      document.body.appendChild(s);
    }
    ['scroll', 'click', 'keydown', 'touchstart'].forEach(function(ev) {
      document.addEventListener(ev, loadChatbot, { once: true, passive: true });
    });
    /* Also load on "Asistanınız" button click immediately */
    var chatBtns = document.querySelectorAll('.alt-btn--chat, [onclick*="MedosChat"]');
    chatBtns.forEach(function(btn) {
      btn.addEventListener('click', loadChatbot, { once: true });
    });
  }
  ...
  function initNav() {
    ...
    document.querySelectorAll('.lang-toggle-btn').forEach(function (btn) {
      btn.addEventListener('click', function () {
        if (window.MEDOS) window.MEDOS.setLang(btn.dataset.setLang);
      });
    });
